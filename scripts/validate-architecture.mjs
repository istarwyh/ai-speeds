...
async function validateRepository(root, manifest) {
  ...
  function exactFacadeMatchCount(facade) {
    ...
    if (facade.kind === 'client-redirect') {
      ...
      if (
        exactSourceContract &&
        ownerFacts?.tokenSignature === sourceTokenSignature(facade.owner, exactSourceContract)
      ) {
        return 1;
      }
...
async function runSelfTest() {
  ...
  const manifestCases = [
    ...
    },
    {
      ...
      mutate(candidate) {
        const exception = requiredFixtureValue(
          candidate.apiToMainExceptions[0],
          'first mutable API-to-route exception',
        );
