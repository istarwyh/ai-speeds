...
- ✅ 项目结构调整
  - 将 `app/` 移到 `src/app/`
  - 将 `components/` 移到 `src/components/`
  - 将 `lib/` 移到 `src/lib-next/`
  - 更新 Tailwind 配置路径

- ✅ 复用现有 API 逻辑
  - 保留 `src/services/llm-provider/adapters/` (format.ts, stream.ts)
  - 保留 `src/services/llm-provider/providers.ts`
  - 保留 `src/services/llm-provider/types.ts`
...
1. **复用现有逻辑** - 直接导入 `@/services/llm-provider/*` 模块，避免重复代码
