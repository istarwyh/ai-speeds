...
- **Find function calls:** `ast-grep --pattern 'functionName($$$)' --lang js .`
...
> **Note**: Language aliases (like `js` for `javascript`) and file extension
> mappings can be customized using the `languageGlobs` configuration.
