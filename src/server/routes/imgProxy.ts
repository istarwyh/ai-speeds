...
export async function handleImgProxy(url: URL, request: Request, _env: Env): Promise<Response> {
  ...
  if (!src) {
    return new Response('Missing src', {
      status: 400,
      headers: { 'Content-Type': 'text/plain; charset=UTF-8', 'X-ImgProxy-Reason': 'missing-src' },
    });
  }
  ...
  try {
    ...
    return new Response('Invalid src', {
      status: 400,
      headers: { 'Content-Type': 'text/plain; charset=UTF-8', 'X-ImgProxy-Reason': 'invalid-src' },
    });
  }

  if (target.protocol !== 'https:') {
    return new Response('Only https is allowed', {
      status: 400,
      headers: { 'Content-Type': 'text/plain; charset=UTF-8', 'X-ImgProxy-Reason': 'non-https' },
    });
  }
  ...
  if (!allowed) {
    return new Response('Host not allowed', {
      status: 403,
      headers: { 'Content-Type': 'text/plain; charset=UTF-8', 'X-ImgProxy-Reason': 'denied-host' },
    });
  }
  ...
  if (!upstream.ok) {
    return new Response(`Upstream ${upstream.status}`, {
      status: 502,
      headers: { 'Content-Type': 'text/plain; charset=UTF-8', 'X-ImgProxy-Reason': 'bad-upstream' },
    });
  }
