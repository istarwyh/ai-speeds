...
基于 Claude
Code，我综合了最佳实践形成了自己的工作流如下，整个过程其实我只需要提出需求以及Review。
...
2. **无限制模式**：在子工作区中每个分支启动 Claude 无限制模式
   ` --dangerously-skip-permissions`，通过 `Shift Tab` 需要时切换模式
3. **Issue 管理**：对于新需求创建 issue，或者拉取当前 issue。多个 issue 可以使用 GitHub
   Projects 管理
4. **任务分派**：按照 issue 给 Claude
   Code 派活。对于复杂需求，使用探索模式理清需求，尽量产出基于 TDD 的技术方案
