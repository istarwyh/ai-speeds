...
Claude
Code 之所以让人愉悦，关键在于它对 LLM 能力的深刻理解：**知道模型擅长什么，不擅长什么**。其提示词和工具设计巧妙地弥补了模型的愚蠢，同时让它在擅长的领域发光发热。
...
Claude
Code 只有一个主线程，使用简单的提示词定期总结 git 历史、合并消息历史。处理复杂任务时，通过克隆自身作为子代理，但**最多只有一个分支**。
...
**不要**：大杂烩的 Do/Don't 列表 **应该**：清晰的决策流程图
...
Claude
Code 的设计非常固执己见，但正是这种固执让它如此优秀。学习这些模式，形成你自己的设计哲学。
