...
function getStatusBadge(isDirectlyUsable: boolean, providerId: string): string {
  ...
  const specialBadge = providerId === 'anyrouter' ? '<span class="bonus-badge">🎁 $100</span>' : '';

  const specialAiCodeWithBadge =
    providerId === 'aicodewith' ? '<span class="bonus-badge">🎁 2000 Free Credits</span>' : '';

  const specialClaudeCodeBadge = providerId === 'claude-code' ? '<span class="bonus-badge">🎁 4000 积分</span>' : '';
