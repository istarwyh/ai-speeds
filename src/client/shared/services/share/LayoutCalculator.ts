      ...
      if (matchEl) {
        ...
        if (rect.width > 0 && rect.height > 0) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.debug(
              `Canvas size: ${width}x${height} (aspect: ${aspect.toFixed(2)}) from page: ${rect.width.toFixed(1)}x${rect.height.toFixed(1)} (dpr:${dpr})`,
            );
          }
